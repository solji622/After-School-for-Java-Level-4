...
public class Ex2_numBaseball {
    public static void main(String[] args) {

        Scanner sc = new Scanner(System.in);

        int n = sc.nextInt(); // 몇 번 시도할 것인지 입력
        int[][] q = new int[n][3]; // 시도한 숫자, 스트라이크, 볼
        for (int i = 0; i < n; i++) {
            q[i][0] = sc.nextInt();
            q[i][1] = sc.nextInt();
            q[i][2] = sc.nextInt();
        ...
        // 3중 for문으로 모든 값을 체크함. 단, 123~987 중에 중복 없는 것만!
        for (int x = 1; x <= 9; x++)
            for (int y = 1; y <= 9 ; y++)
                for (int z = 1; z <= 9; z++) {
                    // 중복은 검사 제외
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        // 입력 받은 스트라이크와 볼의 결과가 다른 경우는 답이 될 수 없음
